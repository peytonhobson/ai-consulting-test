...
async def generate_document_embeddings(texts: List[str]) -> List[List[float]]:
    ...
    try:
        # Check if openai_embeddings_client.embed_documents is already awaitable
        if hasattr(openai_embeddings_client.embed_documents, "__await__"):
            print('awaiting')
            # If it's awaitable, use await
